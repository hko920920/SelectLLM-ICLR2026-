...
OUTPUT = ROOT / f"STEP104_PRIMARY_INDEPENDENT_VALIDATION_{DATE}.json"
...
def main() -> None:
    ...
    checks["all_preoutcome_hashes_still_match"] = all(
    ...
    checks["development_provenance_retained"] = (
        ledger["provenance"]["development_informed_heldout_confirmation"] is True
        and ledger["provenance"]["before_all_data_preregistered"] is False
        and lock["step100_decision"] == "NO_GO_STEP100_GEOMETRY_STOP"
        and lock["step101_decision"] == "NO_GO_STEP101_PARENT_GEOMETRY_STOP"
        and lock["step102_decision"] == "NO_GO_STEP102_LEARNED_DEVELOPMENT_STOP"
        and lock["step103_decision"] == "GO_STEP103_TO_HELDOUT_LOCK"
    )
    ...
    texts = [str(row["text"]) for row in input_payload["rows"]]
    raw_root_columns: list[np.ndarray] = []
    for spec in common.ROOT_SPECS:
        column, _ = common.infer_root_predictions(spec, texts)
    ...
    for relative in lock["adapter_paths"]:
        model, tokenizer = common.load_adapter_model(ROOT / relative)
        raw_score_columns.append(common.infer_error_scores(model, tokenizer, texts))
        model.cpu()
        del model, tokenizer
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    raw_scores = np.stack(raw_score_columns, axis=1).astype(np.float64)
    raw_aliases, _ = common.make_alias_codes(raw_roots[:, 0], raw_scores, thresholds.tolist())
    checks["raw_input_root_and_alias_predictions_bitwise_reconstructed"] = (
        np.array_equal(raw_roots, predictions) and np.array_equal(raw_aliases, aliases)
    )
    ...
    clean = common.run_active(
        clean_codes, labels, clean_parents, clean_codes, pools, seeds, 5, 0.05
    )
    refined = common.run_active(
        refined_codes, labels, refined_parents, clean_codes, pools, seeds, 5, 0.05
    )
    ...
    gates: dict[str, bool] = {
        "lock_and_all_bound_hashes_match": checks["all_preoutcome_hashes_still_match"],
    ...
    output: dict[str, Any] = {
        "validation_id": "STEP104_PRIMARY_INDEPENDENT_VALIDATION_V1",
        "date": DATE,
        "decision": "PASS_STEP104_PRIMARY_INDEPENDENT_RECONSTRUCTION" if not failed else "FAIL_STEP104_PRIMARY_INDEPENDENT_RECONSTRUCTION",
        "checks": checks,
