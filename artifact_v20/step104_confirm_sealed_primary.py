...
OUTPUT_ARRAYS = ROOT / f"STEP104_PRIMARY_CONFIRMATORY_ARRAYS_{DATE}.npz"
OUTPUT_LEDGER = ROOT / f"STEP104_PRIMARY_CONFIRMATORY_LEDGER_{DATE}.json"


def verify_lock(lock: dict[str, Any]) -> None:
    for group in (
        "authority_sha256", "code_sha256", "model_sha256",
        "model_audit_sha256", "preoutcome_sha256",
    ):
    ...
    if common.sha256_path(ROOT / lock["sealed_file"]) != lock["sealed_sha256"]:
        raise AssertionError("sealed bytes drift")
...
def main() -> None:
    if OUTPUT_ARRAYS.exists() or OUTPUT_LEDGER.exists():
        raise FileExistsError("Step 104 primary already evaluated")
    ...
    if preledger["sealed_outcome_loaded"] is not False:
        raise AssertionError("pre-outcome status drift")
    ...
    reconstructed_aliases, reconstructed_triggers = common.make_alias_codes(
        root_predictions[:, 0], error_scores, thresholds.tolist()
    )
    if not np.array_equal(aliases, reconstructed_aliases) or not np.array_equal(triggers, reconstructed_triggers):
        raise AssertionError("pre-outcome alias reconstruction drift")

    # First and only label-array load authorized by the pre-outcome lock.
    sealed = np.load(ROOT / lock["sealed_file"], allow_pickle=False)
    labels = sealed["labels"].astype(np.int64)
    if not np.array_equal(source_indices, sealed["source_indices"].astype(np.int64)):
        raise AssertionError("source-index alignment drift")
    if not np.array_equal(uid_sha256, sealed["uid_sha256"].astype("U64")):
        raise AssertionError("uid alignment drift")
    n = len(labels)
    if n != 13000 or root_predictions.shape != (n, common.ROSTER_SIZE):
        raise AssertionError("held-out shape drift")
    ...
    allowed_loss_count = math.floor(0.01 * n + 1e-12)
    ...
    if seeds != common.TEST_SEEDS or len(seeds) != 3000:
    ...
    inference = {
        "terminal": common.effect_summary(terminal, 100800, 100801),
        "active_minus_fixed_terminal": common.effect_summary(active, 100801, 100802),
        "cumulative": common.effect_summary(cumulative, 100802, 100803),
    }
    ...
    forbidden = {
        ...
        "pools", "posterior", "trajectory", "selector_state", "dataset_id",
    }
    gates: dict[str, bool] = {
        "lock_and_all_bound_hashes_match": True,
        "development_provenance_and_selection_bound": lock["step103_decision"] == "GO_STEP103_TO_HELDOUT_LOCK"
        and lock["selected_cell"] == "l0.0075_t0.025_b5_tau0.050",
        "preoutcome_predictions_bound_and_reconstructed": preledger["prediction_sha256"] == common.sha256_path(PREOUTCOME),
        "four_distinct_14767874_parameter_adapters": len(set(lock["model_sha256"].values())) == common.N_ALIASES
        and all(row["checkpoint"]["parameter_count"] == 14_767_874 for row in step102["adapter_audits"]),
        "raw_input_endpoint_has_no_forbidden_runtime_input": endpoint_parameters == [
            "texts", "parent_predictions", "adapter_paths", "thresholds"
        ] and endpoint_names.isdisjoint(forbidden),
    ...
    ledger: dict[str, Any] = {
        "ledger_id": "STEP104_IMDB_HELDOUT_LEARNED_SAME_S_PRIMARY_V1",
        "date": DATE,
        "decision": decision,
        "failed_gates": failed,
        "gates": gates,
        "provenance": {
            ...
            "step100_decision_retained": lock["step100_decision"],
            "step101_decision_retained": lock["step101_decision"],
            "step102_decision_retained": lock["step102_decision"],
            "step103_decision": lock["step103_decision"],
        },
        "lock_file": LOCK.name,
        ...
        "root_accuracy": np.mean(root_predictions == labels[:, None], axis=0).tolist(),
    ...
    print(json.dumps({
        ...
        "allowed_loss_count": allowed_loss_count,
        "root_accuracy": ledger["root_accuracy"],
