"""Create and stream-audit the deterministic Step 93 / V14 review ZIP."""
...
SOURCE = ROOT / "STEP93_ANONYMOUS_ARTIFACT_RELEASE_CANDIDATE_V14"
MANIFEST_NAME = "STEP82_RELEASE_MANIFEST.json"
ARCHIVE = ROOT / "STEP93_ANONYMOUS_REVIEW_ARTIFACT_V14_2026-08-13.zip"
SIDECAR = ROOT / f"{ARCHIVE.name}.sha256"
AUDIT = ROOT / "STEP93_V14_ZIP_STREAM_AUDIT_2026-08-13.json"
...
def main() -> None:
    ...
    if manifest.get("schema") != "step93.anonymous_validation_closure_release.v8":
        raise AssertionError("unexpected V14 manifest schema")
    ...
    with zipfile.ZipFile(ARCHIVE, "r") as bundle:
        ...
        if duplicates or unexpected or missing:
            failures.append(
                {"duplicates": duplicates, "unexpected": unexpected, "missing": missing}
            )
        ...
        for relative, record in records.items():
            ...
            if total != record["bytes"] or digest.hexdigest() != record["sha256"]:
                failures.append(
                    {"file": relative, "bytes": total, "sha256": digest.hexdigest()}
                )
            else:
                matched += 1
    ...
    report = {
        "verdict": (
            "PASS_STEP93_V14_ZIP_STREAM_AUDIT"
            if not failures
            else "FAIL_STEP93_V14_ZIP_STREAM_AUDIT"
        ),
